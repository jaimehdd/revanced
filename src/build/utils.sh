...
patch() {
	...
	if [ -f "./download/$1.apk" ]; then
		...
		if [ "$3" = inotia ]; then
			...
			p="patch " b="-p *.mpp" m="" a="" ks=" --keystore=./src/morphe.keystore --keystore-password=android --keystore-entry-password=android" pu="--purge=true" opt="" force=" --force"
